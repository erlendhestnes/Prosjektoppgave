...
Current consumption is usually the only measure that is listed in a components datasheet. Which voltage level the vendor decides to test the components current consumption at can vary quite a bit, as seen from the overview in Table \ref{tab:accel_comparison}. A decrease in the operating voltage directly reduces the current consumption and the overall power drain, which means that current consumption can be quite misleading to compare \cite[~p.3]{holberg06}. To avoid an unfair comparison between the devices, the best course of action would have been to measure the current consumption of all devices at the same operating voltage. Another possibility would also have been to test the current consumption for each device at its lowest operating voltage, as this would have given us figures for the lowest possible power consumption for each device. Both of these possibilities would however require a reference board for each and every component, as well as testing. This was deemed to be out of scope for this specialization project. Fortunately, the test supply voltage is around $2V$ for each device in Table \ref{tab:accel_comparison}. The largest test voltage span is between the MMA8491Q and the MC3610.

Power consumption is the product of operating voltage and current consumption. Since 
